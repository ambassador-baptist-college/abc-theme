			...
			if ( $ticket->date_in_range( current_time( 'timestamp' ) ) ) {
				...
				if ( 0 !== $available ) {
					...
					if ( $available ) {
						...
						echo sprintf(
							// Translators: %1: number of tickets available.
							esc_html__( '%1$s available', 'event-tickets-plus' ),
				...
				echo '<td class="tickets_name">' . esc_attr( $ticket->name ) . '</td>';

				echo '<td class="tickets_price">';

				if ( method_exists( $product, 'get_price' ) && $product->get_price() ) {
					echo $this->get_price_html( $product ); // WPCS: XSS ok.
				} else {
				...
				echo '<td class="tickets_description">' . ( $ticket->show_description() ? $ticket->description : '' ) . '</td>'; // WPCS: XSS ok.
