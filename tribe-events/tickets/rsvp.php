		...
		foreach ( $tickets as $ticket ) {
			...
			<tr>
				<td class="tribe-ticket quantity" data-product-id="<?php echo esc_attr( $ticket->ID ); ?>">
					...
					<?php
						// Translators: %1$s: remaining tickets; %2$s: total capacity.
						echo sprintf( esc_html__( '%1$s out of %2$s available', 'event-tickets' ), esc_attr( $ticket->remaining() ), esc_attr( $ticket->capacity() ) );
					?>
				</span>
						<?php endif; ?>
					<?php else : ?>
				...
				<td colspan="4" class="tribe-tickets-attendees">
					...
					<table class="tribe-tickets-table">
						<tr class="tribe-tickets-full-name-row">
							...
							<td colspan="3">
								<input type="text" name="attendee[full_name]" id="tribe-tickets-full-name" placeholder="John Doe">
							</td>
						</tr>
						<tr class="tribe-tickets-email-row">
							...
							<td colspan="3">
								<input type="email" name="attendee[email]" id="tribe-tickets-email" placeholder="john.doe@example.com">
							</td>
						</tr>
